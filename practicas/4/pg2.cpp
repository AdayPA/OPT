  ...
  while ((graph.dirigido == 0 || graph.dirigido == 1) && errorapertura == 0){
    if (graph.dirigido == 0){
      while (option != 'q' && graph.dirigido == 0){
        ...
        cin >> option;
          ...
          switch (option){
            case 'c': graph.actualizar(nombrefichero, errorapertura);
              while (errorapertura == 1){
              graph.actualizar (nombrefichero, errorapertura);
              }
              break;
            case 'i': graph.Info_Grafo();
              break;
            case 'a': graph.Mostrar_Listas(2);
              break;
            case 'o': graph.Compconex();
              break;
    ...
    if (graph.dirigido == 1 && errorapertura == 0){
        ...
        switch (option){
          ...
          case 'i': graph.Info_Grafo();
            break;
          case 's': graph.Mostrar_Listas(1);
            break;
          case 'p': graph.Mostrar_Listas(0);
            break;
          case 'q': cout << "\nGracias por utilizar este programa.\n" << endl;
            graph.dirigido = 2;
        }
